 ...
 *
 * onPreBuild:
 *   1. Parse all /blog/*.md files (frontmatter + body)
 *   2. Write /blog/posts.json  (with clean `url` field)
 *   3. Write /blog/[url-slug]/index.html for each post (pre-rendered, SEO-friendly)
 *
 * Slug resolution:
 *   - Use frontmatter `slug` if present
 *   - Otherwise strip YYYY-MM-DD- prefix from filename and sanitise
 */
...
/* ── Markdown → HTML + section numbering ───────────────────────────────── */

function processArticle(marked, markdown) {
...
function buildRelatedMini(allPosts, currentUrlSlug) {
  ...
  const items = others.map(p => {
    ...
    return `<li class="related-mini-item">
      <a href="../${p.urlSlug}/">
...
<head>
  ...
  <style>
    ...
    .post-author-sub{font-size:12px;color:var(--color-text-muted);display:flex;align-items:center;gap:8px}
...
module.exports = {
  onPreBuild: async ({ utils }) => {
    ...
    try {
      ...
      utils.build.failBuild('Could not require marked. Ensure package.json lists it as a dependency.');
      return;
    }
    ...
    parsedPosts.forEach(post => {
      try {
        const { html: articleHTML, sections } = processArticle(marked, post.body);
